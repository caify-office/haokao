...
namespace HaoKao.CouponService.Application;
...
public class QRCodeHelper
{
    ...
    /// <param name="n1">缩放比例</param>
    /// <returns></returns>
    public static Bitmap MergeQrImg(Bitmap qrImg, Bitmap headerImg, double n1 = 0.23)
    {
        int margin = 0;
    ...
    /// 返回文件
    /// </summary>
    /// <param name="stream11"></param>
    /// <param name="v1"></param>
    /// <param name="v2"></param>
    /// <returns></returns>
    public static FileResult File(Stream stream11, string v1, string v2)
    {
        return File(stream11, "application/x-jpg", $"{DateTime.Now}.jpg");
    }
    ...
    public static FileResult ExportInstitutions(Guid id)
    {
        //图片需要找产品提供
        WebRequest request1 = WebRequest.Create("http://www.haokao123.com/Resource/images/logozb.png");
        WebResponse response1 = request1.GetResponse();
        Stream s1 = response1.GetResponseStream();
        byte[] data1 = new byte[1024];
        int length1 = 0;
        MemoryStream ms1 = new MemoryStream();
        while ((length1 = s1.Read(data1, 0, data1.Length)) > 0)
        {
            ms1.Write(data1, 0, length1);
        }
        ms1.Seek(0, SeekOrigin.Begin);
        byte[] bytes11 = new byte[ms1.Length];
        ms1.Read(bytes11, 0, bytes11.Length);
        ms1.Seek(0, SeekOrigin.Begin);
        var bytes = new Bitmap(new Bitmap(ms1));
        Bitmap bt11;
        //地址需要和前端确认
        bt11 = Create("http://www.haokao123.com/Wechat/PersonalCenter/CardIndex?Code=" + id, 6);
        Stream stream11 = new MemoryStream(Bitmap2Byte(MergeQrImg(bt11, bytes)));
        return File(stream11, "application/x-jpg", $"{DateTime.Now}.jpg");
    }
}