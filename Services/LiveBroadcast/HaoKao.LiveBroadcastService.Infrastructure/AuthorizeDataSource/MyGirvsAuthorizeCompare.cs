...
namespace HaoKao.LiveBroadcastService.Infrastructure.AuthorizeDataSource;

public abstract class MyGirvsAuthorizeCompare : GirvsRepositoryOtherQueryCondition, IServiceMethodPermissionCompare
{
    ...
    public override Expression<Func<TEntity, bool>> GetOtherQueryCondition<TEntity>()
    {
        ...
        if (!IsLogin())
        {
            var tenantId = EngineContext.Current.ClaimManager.IdentityClaim?.TenantId.ToGuid();
        ...
        var dataRuleModels = currentUserAuthorize.AuthorizeDataRules ?? throw new GirvsException("未获取相关的数据授权信息");
        ...
        if (currentEntityDataRule == null || !currentEntityDataRule.AuthorizeDataRuleFieldModels.Any())
        {
            ...
            {
                throw new GirvsException("未配置当前用户对该模块的数据权限，请先获取权限");
            }
        }
        else
        {
    ...
    private Dictionary<string, string[]> entityDataRuleOrFields = new();
    ...
    public virtual bool PermissionCompare(Guid functionId, Permission permission)
    {
        ...
        var ps = currentUserAuthorize.AuthorizePermissions ?? throw new GirvsException("未获取相关的功能授权信息");
