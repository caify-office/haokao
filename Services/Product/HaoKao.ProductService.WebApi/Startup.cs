...
namespace HaoKao.ProductService.WebApi;
    ...
    public void Configure(IApplicationBuilder app, IWebHostEnvironment env)
    {
        ...
        app.Use(async (context, next) =>
        {
            if (context.Request.Path.HasValue
             && (context.Request.Path.Value.ToLower().IndexOf("/api/website/productpackagewebservice") == 0
              || context.Request.Path.Value.ToLower().IndexOf("/api/WebSite/ProductWebService/ByIds") == 0
                )
               )
            {
                //如果当前没有登陆,设置为临时用户
