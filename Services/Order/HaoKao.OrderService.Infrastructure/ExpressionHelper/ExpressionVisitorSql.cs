...
{
    protected ExpressionVisitorSql() { }
...
public class PartialEvaluator(Func<Expression, bool> fnCanBeEvaluated) : ExpressionVisitorSql
{
    ...
    {
        m_candidates = new Nominator(fnCanBeEvaluated).Nominate(exp);
...
internal class ConditionBuilder : ExpressionVisitorSql
{
    ...
    protected override Expression VisitMemberAccess(MemberExpression m)
    {
        ...
        {
            m_conditionParts.Push(String.Format(" {0} ", AddQuotationMarks(propertyInfo.Name)));
        }
        else
        {
    ...
    static string BinarExpressionProvider(Expression left, Expression right, ExpressionType type)
    {
        ...
        {
            if (sb.EndsWith(" ="))
                sb = string.Concat(sb.AsSpan(0, sb.Length - 1), " is null");
            else if (sb.EndsWith("<>"))
                sb = string.Concat(sb.AsSpan(0, sb.Length - 1), " is not null");
        }
        else
            sb += tmpStr;
        return sb += ")";
