...
namespace HaoKao.KnowledgePointService.WebApi;
...
public class Startup(IConfiguration configuration, IWebHostEnvironment webHostEnvironment):IGirvsStartup
{
    ...
    {
     /*   services.AddDbContext<KnowledgePointDbContext>(options =>
  options.UseMySql(this.Configuration.GetConnectionString("DefaultConnection")));*/
/*        services.AddDbContext<KnowledgePointDbContext>();*/
        ...
        //services.AddEntityFrameworkMySql().AddDbContext(options => options.UseMySql(Configuration["sqlserver"]));
    }
