...
namespace HaoKao.TenantService.Infrastructure.AuthorizeDataSource;
...
public class AuthorizeRepository(IAuthorityNetWorkRepository authorityNetWorkRepository, IStaticCacheManager staticCacheManager) : GirvsAuthorizeCompare
{
    ...
    {
        var key = $"{GirvsAuthorizePermissionCacheKeyManager.CurrentUserAuthorizeCacheKeyPrefix}:{EngineContext.Current.ClaimManager?.IdentityClaim.UserId ?? "临时用户"}";
