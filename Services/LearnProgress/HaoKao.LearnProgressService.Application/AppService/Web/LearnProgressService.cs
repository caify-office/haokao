...
public class LearnProgressService(
    ...
    ILearnProgressService learnProgressService
) : ILearnProgressWebService
{
    #region 初始参数
    ...
    public  Task<float> GetLearnDurations(Guid productId, Guid subjectId)
    {
        ...
        return learnProgressService.GetLearnDurations(productId, subjectId, userId);
    }
    /// <summary>
    ...
    public async Task<LearnProgressQueryViewModel> GetList([FromBody] LearnProgressQueryViewModel queryViewModel)
    {
        ...
        result.Result = result.Result.GroupBy(obj => obj.VideoId)
                 .Select(group => group.OrderByDescending(obj => obj.CreateTime).ThenByDescending(obj=>obj.IsEnd).FirstOrDefault()).ToList();
        return result;
    }
