...
namespace HaoKao.OpenPlatformService.Application.IdentityServerExtensions.OAuth.HaokaoGithub;

public class HaoKaoGitHubAuthenticationHandler(
    IOptionsMonitor<GitHubAuthenticationOptions> options,
    ILoggerFactory logger,
    UrlEncoder encoder)
    : GitHubAuthenticationHandler(options, logger, encoder), IHaoKaoAuthenticationHandler
{
    ...
    public ExternalUser GetCurrentAuthenticationUniqueIdentifier(IEnumerable<System.Security.Claims.Claim> claims)
    {
        ...
        {
            Schemem = GitHubAuthenticationDefaults.AuthenticationScheme,
            NikeName = nikeName,
